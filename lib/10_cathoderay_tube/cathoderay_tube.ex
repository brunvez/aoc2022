defmodule AOC2022.CathoderayTube do
  @moduledoc """
  ...
  screen and simple CPU that are both driven by a precise clock circuit. The clock circuit ticks at a constant rate; each tick is called a cycle.
  ...
  def solve(input, process_cycles) do
    input
    |> parse()
  ...
  def get_signal_strengths(cycles) do
    cycles
    |> Enum.drop(20)
    |> Enum.take_every(40)
    |> Enum.reduce(0, fn {x, cycle}, sum -> sum + x * cycle end)
  end

  def draw_pixels(cycles) do
    cycles
    |> Enum.drop(1)
    |> Enum.map(fn {x, cycle_number} ->
      cycle_number = Integer.mod(cycle_number, 40)

      if cycle_number in x..(x + 2) do
        "#"
      else
        "."
      end
  ...
  defp parse(input) do
    ...
    |> Enum.map(fn
      ["addx", number] -> {:addx, String.to_integer(number)}
      ["noop"] -> :noop
    end)
  end
  ...
  defp execute_instructions(instructions) do
    instructions
    |> Enum.reduce([1], fn
      {:addx, v}, [{:add, x, v2} | cycles] ->
        updated_x = x + v2
        [{:add, updated_x, v}, updated_x, updated_x] ++ cycles
    ...
    |> Enum.map(&maybe_add/1)
    |> Enum.reverse()
    |> Enum.with_index()
  end

  defp maybe_add({:add, x, v}), do: x + v
  defp maybe_add(x), do: x
end

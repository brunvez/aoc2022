defmodule AOC2022.MonkeyInTheMiddle do
  @moduledoc """
  ...
  how worried you are about those items, and how the monkey makes decisions based on your worry level. For example:
  ...
  defmodule Monkey do
    ...
    def start_link(%__MODULE__{id: id} = initial_value) do
      Agent.start_link(fn -> initial_value end, name: :"#{__MODULE__} #{id}")
    end

    def state(monkey_agent) do
      Agent.get(monkey_agent, & &1)
    end

    def add_item(monkey_agent, item) do
      Agent.update(monkey_agent, fn %Monkey{held_items: held_items} = monkey ->
        %{monkey | held_items: held_items ++ [item]}
      end)
    end

    def inspect_items(monkey_agent) do
      %Monkey{
        ...
        monkey_if: monkey_if,
        monkey_else: monkey_else,
        max_number: max_number
      } = Agent.get(monkey_agent, & &1)

      items =
        Enum.map(held_items, fn item ->
          item = operation.(item)
          item_to_send = item |> div(worry_divisor) |> Integer.mod(max_number)

          if Integer.mod(item_to_send, divisor) == 0 do
            {item_to_send, monkey_if}
          else
            {item_to_send, monkey_else}
          end
        end)

      Agent.update(monkey_agent, fn %Monkey{inspected_items: inspected_items} = monkey ->
        %{monkey | held_items: [], inspected_items: inspected_items + length(items)}
      end)
  ...
  def solve(input, worry_divisor: worry_divisor, rounds: rounds) do
    input
    |> parse(worry_divisor)
    |> set_max_number()
    |> pass_items(rounds)
  ...
  defp parse(input, worry_divisor) do
    input
    ...
    |> Enum.reject(&(&1 == ""))
    |> Enum.chunk_every(6)
    |> Enum.map(&init_monkey(&1, worry_divisor))
  end

  defp pass_items(monkeys, rounds) do
    monkey_agents =
      Enum.map(monkeys, fn %Monkey{} = monkey ->
        {:ok, monkey_agent} = Monkey.start_link(monkey)
        monkey_agent
      end)

    Enum.each(1..rounds, fn _round ->
      Enum.each(monkey_agents, fn current_monkey ->
        inspected_items = Monkey.inspect_items(current_monkey)

        Enum.each(inspected_items, fn {item, monkey_id} ->
          monkey_to = Enum.at(monkey_agents, monkey_id)
          Monkey.add_item(monkey_to, item)
        end)
      end)
    end)

    Enum.map(monkey_agents, fn agent -> Monkey.state(agent) end)
  end

  defp init_monkey(input, worry_divisor) do
    [
      "Monkey " <> <<id::bytes-size(1)>> <> ":",
      "Starting items: " <> items,
      "Operation: new = " <> operation,
      "Test: divisible by " <> divisor,
      "If true: throw to monkey " <> monkey_if,
      "If false: throw to monkey " <> monkey_else
    ] = input

    held_items = items |> String.split(", ") |> Enum.map(&String.to_integer/1)

    %Monkey{
      id: String.to_integer(id),
      held_items: held_items,
      operation: operation_to_fn(operation),
      divisor: String.to_integer(divisor),
      worry_divisor: worry_divisor,
      monkey_if: String.to_integer(monkey_if),
      monkey_else: String.to_integer(monkey_else)
    }
  end
  ...
  defp operation_to_fn("old + " <> num) do

defmodule AOC2022.SupplyStacks do
  @moduledoc """
  --- Day 5: Supply Stacks ---
  ...
  To ensure none of the crates get crushed or fall over, the crane operator will rearrange them in
  a series of carefully-planned steps. After the crates are rearranged, the desired crates will be at the top of each stack.
  ...
  [Z] [M] [P]
  1   2   3
  In the second step, three crates are moved from stack 1 to stack 3. Crates are moved one at a time, so the first crate to be moved (D)
  ...
  defmodule Instruction do
    defstruct [:from, :to, :pieces]
  end

  def solve(input, moving_function) do
    input
    |> parse()
    |> apply_instructions(moving_function)
    |> Map.values()
    |> Enum.map(&List.first/1)
    |> Enum.join()
  end

  def move_individually(column, pieces) do
    column
    |> Enum.take(pieces)
    |> Enum.reverse()
  end

  def move_groupally(column, pieces) do
    column
    |> Enum.take(pieces)
  end
  ...
  defp parse(input) do
    ...
    instructions =
      lines
      |> Enum.drop_while(&(!String.starts_with?(&1, "move")))
      |> Enum.map(&parse_instruction/1)

    {stack, instructions}
  end

  defp parse_stack(lines) do
    lines
    |> Enum.map(&String.graphemes/1)
    |> Enum.map(fn [_ | line] ->
      Enum.take_every(line, 4)
    end)
    |> transpose()
    |> Enum.map(fn column -> Enum.reject(column, &(&1 == " ")) end)
    |> Enum.with_index()
    |> Map.new(fn {column, index} -> {index + 1, column} end)
  end

  defp parse_instruction(instruction) do
    %{"from" => from, "to" => to, "pieces" => pieces} =
      Regex.named_captures(~r/move (?<pieces>\d+) from (?<from>\d+) to (?<to>\d+)/, instruction)

    %Instruction{
      from: String.to_integer(from),
      to: String.to_integer(to),
      pieces: String.to_integer(pieces)
    }
  end

  defp apply_instructions({stack, instructions}, moving_function) do
    Enum.reduce(instructions, stack, fn %Instruction{from: from, to: to, pieces: pieces}, stack ->
      items = moving_function.(stack[from], pieces)

      stack
      |> Map.update!(from, &Enum.drop(&1, pieces))
      |> Map.update!(to, &(items ++ &1))
    end)
  end

defmodule AOC2022.CalorieCounter do
  @moduledoc """
  ...
  to know how many Calories are being carried by the Elf carrying the most Calories. In the example above, this is 24000 (carried by the fourth Elf).
  ...
  the top three Elves carrying the most Calories. That way, even if one of those Elves runs out of snacks, they still have two backups.

  In the example above, the top three Elves are the fourth Elf (with 24000 Calories), then the third Elf
  (with 11000 Calories), then the fifth Elf (with 10000 Calories). The sum of the Calories carried by these three elves is 45000.

  Find the top three Elves carrying the most Calories. How many Calories are those Elves carrying in total?
  """

  def solve(calories, top: top) do
    calories
    |> parse()
    |> Enum.map(&Enum.sum/1)
    |> Enum.sort(:desc)
  ...
  defp parse(calories) do
    ...
    |> Enum.chunk_while(
      ...
      fn
        "", acc -> {:cont, acc, []}
        calorie, acc -> {:cont, [parse_calorie(calorie) | acc]}
      end,
      fn [] -> {:cont, []} end
    )
  end

  defp parse_calorie(calorie) do
    calorie
    |> String.trim()
    |> String.to_integer()
  end

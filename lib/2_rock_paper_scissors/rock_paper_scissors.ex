defmodule AOC2022.RockPaperScissors do
  @moduledoc """
  ...
  def solve(input, round_converter) do
    input
    |> String.trim()
    |> String.split("\n")
    |> Enum.map(&String.split(&1, " "))
    |> Enum.map(&List.to_tuple/1)
    |> Enum.map(round_converter)
    |> Enum.map(&score/1)
    |> Enum.sum()
  end

  def given_shape_converter({opponent_shape, "X"}), do: {opponent_shape, "A"}
  def given_shape_converter({opponent_shape, "Y"}), do: {opponent_shape, "B"}
  def given_shape_converter({opponent_shape, "Z"}), do: {opponent_shape, "C"}

  def given_outcome_converter({"A" = opponent_shape, "X"}), do: {opponent_shape, "C"}
  def given_outcome_converter({"A" = opponent_shape, "Y"}), do: {opponent_shape, "A"}
  def given_outcome_converter({"A" = opponent_shape, "Z"}), do: {opponent_shape, "B"}
  def given_outcome_converter({"B" = opponent_shape, "X"}), do: {opponent_shape, "A"}
  def given_outcome_converter({"B" = opponent_shape, "Y"}), do: {opponent_shape, "B"}
  def given_outcome_converter({"B" = opponent_shape, "Z"}), do: {opponent_shape, "C"}
  def given_outcome_converter({"C" = opponent_shape, "X"}), do: {opponent_shape, "B"}
  def given_outcome_converter({"C" = opponent_shape, "Y"}), do: {opponent_shape, "C"}
  def given_outcome_converter({"C" = opponent_shape, "Z"}), do: {opponent_shape, "A"}

  defp score({_, own_shape} = round), do: outcome_score(round) + shape_score(own_shape)
  ...
  defp outcome_score(_round), do: 3

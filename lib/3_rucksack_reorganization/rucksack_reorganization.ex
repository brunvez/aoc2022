defmodule AOC2022.RucksackReorganization do
  @moduledoc """
  ...
  which means its first compartment contains the items vJrwpWtwJgWr, while the second compartment contains the items hcsFMMfFFhFp.
  ...
  def solve(input, splitter) do
    input
    |> parse()
    |> Enum.map(&String.codepoints/1)
    |> splitter.()
    |> Enum.flat_map(&find_intersection/1)
    |> Enum.map(&priority/1)
    |> Enum.sum()
  end

  def compartments_splitter(rucksacks) do
    Enum.map(rucksacks, fn rucksack ->
      middle = div(Enum.count(rucksack), 2)
  ...
  def group_splitter(rucksacks) do
  ...
  defp find_intersection(streams) do
    streams
  ...
  defp priority(item) do
    possible_items = Enum.to_list(?a..?z) ++ Enum.to_list(?A..?Z)
    priorities = Enum.map(possible_items, &<<&1::utf8>>)

    Enum.find_index(priorities, &(&1 == item)) + 1
  end
end

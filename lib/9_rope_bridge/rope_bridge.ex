defmodule AOC2022.RopeBridge do
  @moduledoc """
  --- Day 9: Rope Bridge ---
  ...
  of the knots on a two-dimensional grid. Then, by following a hypothetical series of motions (your puzzle input)
  ...
  --- Part Two ---
  A rope snaps! Suddenly, the river is getting a lot closer than you remember. The bridge is still there, but some of the ropes
  ...
  def solve(input, knots: knots) do
    input
    |> parse()
    |> execute_motions(knots)
    |> elem(1)
    |> Enum.count()
  end
  ...
  defp execute_motions(motions, knots) do
    motions
    |> Enum.reduce({knots, MapSet.new([Enum.at(knots, -1)])}, fn
      motion, {knots, tail_moves} -> move(motion, knots, tail_moves)
    end)
  end

  defp move({_dir, 0}, knots, tail_moves), do: {knots, tail_moves}

  defp move({dir, n}, [head | knots], tail_moves) do
    [new_tail | _] =
      new_positions =
      Enum.reduce(knots, [do_step(dir, head)], fn
        knot, [prev_knot | _rest] = acc ->
          if touching?(knot, prev_knot) do
            [knot | acc]
          else
            [catch_up(prev_knot, knot) | acc]
          end
      end)
  ...
  defp do_step("R", {x, y}), do: {x + 1, y}
  defp do_step("L", {x, y}), do: {x - 1, y}
  defp do_step("U", {x, y}), do: {x, y - 1}
  defp do_step("D", {x, y}), do: {x, y + 1}
  defp do_step("N", {x, y}), do: {x, y}

  defp catch_up({x1, y1}, {x2, y2}) do
    new_position = do_step(x_dir(x1, x2), {x2, y2})

    do_step(y_dir(y1, y2), new_position)
  end

  defp x_dir(x, x), do: "N"
  defp x_dir(x1, x2) when x1 > x2, do: "R"
  defp x_dir(x1, x2) when x1 < x2, do: "L"

  defp y_dir(y, y), do: "N"
  defp y_dir(y1, y2) when y1 > y2, do: "D"
  defp y_dir(y1, y2) when y1 < y2, do: "U"
